...
- Leader: Sébastien Mosser
  - Features:
    ...
    - **A diagnostic view you can explore:** The diagnostic view is no longer one long wall of text. Problems, models, symbols and the command trace each get their own filterable table, over a summary line that keeps the error count in sight. Click a problem or a symbol to jump straight to the line it names — including into a file your model only `load`s. Macro expansions in the command trace are folded away, so a trace of hundreds of steps shows you the handful of commands you actually wrote, with the rest one click away. Model cards link to the template they implement and the models implementing them, which is the inheritance picture the old text buried in two separate sections. Where you were reading — the tab, the filter, the macros you opened — survives a save. This needs a compiler that can report in a machine-readable form; with an older one the view shows the same text report as before, unchanged and with no error

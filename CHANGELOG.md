...
- Leader: Sébastien Mosser
  ...
  - Bug Fixes:
    - **Typing `@` and accepting the suggestion no longer gives you `@@support`.** Inside a template, `@` can only be the start of `@support`, so the editor offers it immediately — and then inserted it *beside* what you had typed rather than over it, leaving a doubled `@` that does not parse. Typing more of the word had the same result. It now replaces what you typed, whether that is a bare `@` or half the word
  ...
  - Documentation:
    - **The Marketplace page reads as a page again, not a manual.** It had grown a walkthrough of every quick fix and refactoring, the reasoning behind where Organize Loads sits in the menus, and the whole formatter contract — several screens of it, in the place people look to decide whether to install the extension at all. One paragraph now names what you get; the tutorials at [jpipe.org](https://www.jpipe.org/tutorials/), which teach it properly and stay current, are one click away. Nothing about the extension changed, and two typos on that page are gone

...
- Leader: Sébastien Mosser
  - Features:
    ...
    - **Organize loads:** Sort and de-duplicate the `load` statements at the top of a file, from **Source Action…**, the command palette, or on save if you turn it on. It never removes a `load` whose path does not resolve: a half-typed path is exactly the state a file is in while you are writing one

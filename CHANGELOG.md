...
- Leader: Sébastien Mosser
  - Features:
    ...
    - **See what a pattern matched:** Hover a glob to get the list of files it resolved to, each a link you can click through to. A pattern that matches nothing, or that is malformed, is reported with the same wording the compiler uses — and if it points outside the file's own directory (`../elsewhere/*.jd`), the message says so, since that is the one case where a pattern fails although the equivalent single-file path would load

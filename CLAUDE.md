...
Matching is relative to the declaring file's directory; results are sorted; zero matches and malformed patterns are errors worded exactly as the compiler words them. Expansion prunes `node_modules`, `out`, `.git` and dot-directories (the compiler does not, but it runs once per build whereas this runs while the user types), and is memoised, with `DocumentBuilder.onUpdate` clearing the cache.
